...
with tab1:
    st.markdown('<div class="cp-section-head">REAL-TIME DETECTION</div>', unsafe_allow_html=True)
    run_webcam = st.checkbox("⬡ ACTIVATE WEBCAM FEED")

    st.markdown('<div class="cp-webcam-frame">', unsafe_allow_html=True)
    frame_window = st.image([])
    st.markdown('</div>', unsafe_allow_html=True)

    if not run_webcam:
        st.markdown("""
        <div class="cp-info-box" style="margin-top:0.75rem">
          &gt; WEBCAM OFFLINE · CHECK BOX ABOVE TO INITIALIZE LIVE FEED<br>
    ...
    if run_webcam:
        ...
        while run_webcam:
            ret, frame = cap.read()
            if not ret:
                st.error("⚠ FEED LOST — CANNOT ACCESS WEBCAM. CHECK PERMISSIONS.")
                break
            ...
            for (x, y, w, h) in faces:
                ...
                cv2.rectangle(frame, (x, y), (x+w, y+h), (245, 216, 0), 2)
                label = f"{emotion.upper()} {confidence:.1f}%"
                cv2.putText(
                    frame,
                    label,
